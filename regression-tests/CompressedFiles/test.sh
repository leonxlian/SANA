...
"$SANA_EXE" -ec 1 -tolerance 0 -t 1 -fg1 "$REG_DIR/MMusculus.el.gz" -fg2 "$REG_DIR/AThaliana.el.bz2" -o "$REG_DIR/test1" &> "$REG_DIR/test1.progress" &
echo "Test 2: Testing compressed edgelists AThaliana.el.gz and yeast-reduced-wayne.gw.gz"
"$SANA_EXE" -ec 1 -tolerance 0 -t 1 -fg1 "$REG_DIR/yeast-reduced-wayne.gw.gz" -fg2 "$REG_DIR/AThaliana.el.gz" -o "$REG_DIR/test2" &> "$REG_DIR/test2.progress"
