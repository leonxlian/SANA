...
(
    ...
    echo "if ($SANA_EXE -g1 yeast -g2 human -tolerance 0 -t 1 -ec 1 -fcolor1 $REG_DIR/yeast.col -fcolor2 $REG_DIR/human.col -o $TMPDIR/y-h.col >$TMPDIR/y-h.col.stdout 2>&1);then echo yeast-human WITH COLORS SHOULD HAVE FAILED BUT WORKED >&2;exit 1;else exit 0;fi"
    ...
    echo "$SANA_EXE -fg1 $REG_DIR/covid.el -fg2 $REG_DIR/covid.el -tolerance 0 -t 1 -ec 1 -fcolor1 $REG_DIR/covid.col -fcolor2 $REG_DIR/covid.col -o $TMPDIR/covid > $TMPDIR/covid.stdout 2>&1"
    #after running it:
    ...
    echo "if ($SANA_EXE -fg1 $REG_DIR/covid.el -fg2 $REG_DIR/covid.el -tolerance 0 -t 1 -ec 1 -lock-same-names -o $TMPDIR/lock > $TMPDIR/lock.stdout 2>&1);then echo OOPS covid.el with lock-same-names should have failed but worked! >&2;exit 1;else exit 0;fi"
    ...
    echo "$SANA_EXE -fg1 $REG_DIR/colorTest1.el -fcolor1 $REG_DIR/colorTest1.col -fg2 $REG_DIR/colorTest2.el -fcolor2 $REG_DIR/colorTest2.col -ec 1 -tolerance 0 -t 1 -o $TMPDIR/colorTest1 > $TMPDIR/colorTest1.stdout 2>&1"
    ...
    echo "$SANA_EXE -fg1 $REG_DIR/colorTest1.el -fg2 $REG_DIR/colorTest2.el -lock $REG_DIR/lockTest.lock -ec 1 -tolerance 0 -t 1 -o $TMPDIR/colorTest2 > $TMPDIR/colorTest2.stdout 2>&1"
